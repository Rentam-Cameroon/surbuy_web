...
import VerifyContactDialog from "@/components/profile/VerifyContactDialog"
...
export default function ProfilePage() {
    ...
    const [verifyDialog, setVerifyDialog] = useState<{ isOpen: boolean; mode: 'phone' | 'email'; value: string }>({
        isOpen: false,
        mode: 'phone', // default
        value: ''
    })
    ...
    // if (!user) early return removed to allow appearance settings
    ...
    return (
        <div className="pb-32 pt-6 min-h-screen bg-background">
                ...
                {user ? (
                    ...
                    <div className="text-center space-y-6 py-8">
                        <div className="space-y-2">
                            <h1 className="text-2xl font-bold tracking-tight">{t("Login to Surbuy")}</h1>
                            <p className="text-muted-foreground text-sm max-w-xs mx-auto">
                                {t("Access your orders, messages, and profile settings.")}
                            </p>
                        </div>
                        <Button onClick={() => router.push('/register')} className="rounded-full px-8 h-12 text-base font-bold shadow-lg shadow-primary/20">
                            {t("Login / Register")}
                        </Button>
                    </div>
                )}
                                ...
                                <div className="flex items-center justify-between p-4 px-6 hover:bg-muted/10 transition-colors">
                                    ...
                                    {userData.isEmailVerified ? (
                                        ...
                                        <Button
                                            ...
                                            onClick={() => setVerifyDialog({ isOpen: true, mode: 'email', value: userData.email })}
                                        >
                                            {t("Verify Now")}
                                        </Button>
                                    )}
                                ...
                                <div className="flex items-center justify-between p-4 px-6 hover:bg-muted/10 transition-colors">
                                    ...
                                    {userData.isPhoneVerified ? (
                                        ...
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            className="h-7 text-[10px] rounded-full text-amber-600 border-amber-200 bg-amber-50 group hover:bg-amber-100"
                                            onClick={() => setVerifyDialog({ isOpen: true, mode: 'phone', value: userData.phone })}
                                        >
            ...
            <VerifyContactDialog
                ...
                onClose={() => setVerifyDialog(prev => ({ ...prev, isOpen: false }))}
                mode={verifyDialog.mode}
                value={verifyDialog.value}
                onSuccess={() => {
                    // Refresh user data (or just reload page for now)
